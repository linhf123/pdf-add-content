...
import { message, Upload, Row, Col, Modal, Form, Input, Space, Select, Button } from 'antd';
...
const App: React.FC = () => {
  ...
  const { loading, run } = useRequest((data) => {
    ...
    onSuccess: res => {
      ...
      if (res.status === 200) {
        ...
        message.error('上传失败，可以尝试修改表格列名，PDF 暂时不支持中文水印')
      }
    }
  })
  ...
  return (
    <Row gutter={24} style={{ marginTop: '20vh', paddingLeft: '20vw' }} >
      <Col>
        <Dragger
          ...
          beforeUpload={(file) => {
            ...
            console.log(['xls', 'xlsx', 'csv', ...restList], '111111111')
            if (!isExcel) {
      ...
      {
        excelList.length > 0 && <Col>
          <Form form={form} name="validateOnly" autoComplete="off">
            <Form.Item name="columnName" label="表格列名" initialValue={options[0]?.value} required>
              ...
              <Button
                ...
                onClick={() => {
                  form.validateFields().then(({ size, x, y, columnName }) => {
                    ...
                    const columnList = excelList.map(item => item[columnName])
